...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, n_fft,
                 hop_size, win_size, transit_hop_size, sampling_rate, split=True, shuffle=True, n_cache_reuse=1,device=None):
        ...
        self.transit_hop_size = transit_hop_size
        self.cached_wav = None
        self.n_cache_reuse = n_cache_reuse
        self._cache_ref_count = 0
        self.device = device
        self.ratio = self.hop_size // self.transit_hop_size
    ...
    def __getitem__(self, index):
        ...
        log_amplitude, phase = amp_pha_spectra(audio, self.n_fft, self.transit_hop_size, self.win_size) #[1,n_fft/2+1,frames]
        log_amplitude_ = log_amplitude.cpu().numpy()
        log_amplitude_=log_amplitude_[:,:,0::self.ratio]
        log_amplitude_interp = signal.resample(log_amplitude_, log_amplitude_.shape[-1]*self.ratio, axis=2)
        if log_amplitude.size()[-1]%self.ratio>0:
            log_amplitude_interp=log_amplitude_interp[:,:,0:-(log_amplitude.size()[-1]%self.ratio)]
        log_amplitude_interp = torch.FloatTensor(log_amplitude_interp)


        return (log_amplitude_interp.squeeze(), phase.squeeze())
