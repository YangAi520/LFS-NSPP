...
def generation(h):
    ...
    with torch.no_grad():
        for i, filename in enumerate(filelist):
            ...
            log_amplitude_interp = log_amplitude.cpu().numpy()
            log_amplitude_interp = signal.resample(log_amplitude_interp, log_amplitude.size()[2]*(h.hop_size//h.transit_hop_size), axis=2)
            log_amplitude_interp = torch.FloatTensor(log_amplitude_interp).to(device)

            phase_g = NSPP(log_amplitude_interp)
            phase_g = phase_g[:,:,0::(h.hop_size//h.transit_hop_size)]

            min_len=min(log_amplitude.size()[-1],phase_g.size()[-1])
            phase_g=phase_g[:,:,0:min_len]
            log_amplitude=log_amplitude[:,:,0:min_len]
